  ...
  return (
    <Link href={`/lives/${id}`} className="group relative min-w-[280px] max-w-[320px] flex-shrink-0 overflow-hidden rounded-[16px] bg-[#17181B] transition-all duration-300 hover:scale-[1.02] block">
      <div className="relative aspect-video overflow-hidden rounded-[16px] bg-gradient-to-br from-zinc-800 to-zinc-900">
        ...
        <div className="absolute left-2 top-2 rounded-full bg-[#F04A8A]/20 px-2 py-0.5 text-[11px] font-medium text-[#F04A8A]">{when}</div>
      </div>
      <div className="p-3">
        <p className="text-xs font-medium text-[#F04A8A]">{instructor}</p>
        <h3 className="mt-1 text-sm font-medium leading-snug text-white line-clamp-2">{title}</h3>
        ...
        <div className="mt-2 flex items-center justify-between">
          ...
          <div className="inline-flex h-7 items-center gap-1.5 rounded-full px-2 text-xs text-[#F04A8A]">
