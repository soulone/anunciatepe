      ...
      <div className="relative aspect-[2/3] overflow-hidden bg-gradient-to-br from-zinc-100 to-zinc-200">
        {thumbnail ? (
          ...
          <div className="flex h-full w-full items-center justify-center">
            <span className="text-2xl font-bold tracking-wider text-[#F26A2E]/30">{title.split(" ")[0]}</span>
          </div>
        )}
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 p-4 opacity-0 transition-opacity duration-300 group-hover:opacity-100">
          {subtitle && <p className="text-center text-xs text-white">{subtitle}</p>}
          <div className="inline-flex items-center gap-1.5 rounded-full bg-[#F26A2E] px-4 py-1.5 text-xs font-bold text-white">
